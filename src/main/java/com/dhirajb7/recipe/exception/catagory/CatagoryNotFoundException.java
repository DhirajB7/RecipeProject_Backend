package com.dhirajb7.recipe.exception.catagory;

public class CatagoryNotFoundException extends RuntimeException {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	public CatagoryNotFoundException() {
		super();
		// TODO Auto-generated constructor stub
	}

	public CatagoryNotFoundException(String message, Throwable cause, boolean enableSuppression,
			boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
		// TODO Auto-generated constructor stub
	}

	public CatagoryNotFoundException(String message, Throwable cause) {
		super(message, cause);
		// TODO Auto-generated constructor stub
	}

	public CatagoryNotFoundException(String message) {
		super(message);
		// TODO Auto-generated constructor stub
	}

	public CatagoryNotFoundException(Throwable cause) {
		super(cause);
		// TODO Auto-generated constructor stub
	}

}
